export default class Tracker {
    constructor(querySelectorString) {
        ...
        this.root.innerHTML = Tracker.html();
    ...
    load() {
        const entries = JSON.parse(localStorage.getItem("expense-tracker")) || "[]";
    ...
    addEntry(entry = {}) {
        this.root.querySelector(".entries").insertAdjacentHTML("beforeend", Tracker.entryHtml())

        const row = this.root.querySelector(".entries tr:last-of-type");

        row.querySelector(".input-date").value = entry.date || new Date().toISOString(0).replace(/T.*/, "");
